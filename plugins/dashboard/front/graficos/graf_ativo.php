...
<html> 
<head>
<title>GLPI - <?php echo __('Charts','dashboard'). " " . __('by Asset','dashboard'); ?></title>
...
$arr_grp = array();
$arr_grp[0] = "-- ". __('Select a asset type','dashboard') . " --" ;
...
<div id="head" class="row-fluid">
	...
	<div id="titulo_graf">
	   <?php echo __('Tickets','dashboard') ." ". __('by Asset','dashboard'); ?> 
	...
	<div id="datas-tec" class="span12 row-fluid" > 
		<form id="form1" name="form1" class="form2" method="post" action="?date1=<?php echo $data_ini ?>&date2=<?php echo $data_fin ?>&con=1" onsubmit="datai();dataf();"> 
			...
			<button class="btn btn-primary btn-small" type="button" name="Limpar" value="Limpar" onclick="location.href='graf_ativo.php'" > <i class="icon-white icon-trash"></i>&nbsp; <?php echo __('Clean','dashboard'); ?> </button></td>
			</td>
			</tr>	
			</table>
...
if($id_grp == "0") {
	echo '<script language="javascript"> alert(" ' . __('Select a asset type','dashboard') . ' "); </script>';
	echo '<script language="javascript"> location.href="graf_ativo.php"; </script>';
}
